<x-app-layout>
    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6">
         ...
         {{-- ========================================================================= --}}
            {{-- 1. KHỐI THỐNG KÊ TỔNG QUAN (CHỈ DÀNH CHO ADMIN)                             --}}
            {{-- ========================================================================= --}}
            @if($role == 'admin')
            <div class="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
                
                <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700 p-6 transition-all duration-300 hover:shadow-lg hover:-translate-y-1">
                    <div class="flex items-center justify-between">
                        <div>
                            <p class="text-sm font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wider">Tổng Nhân Viên</p>
                            <h3 class="text-3xl font-black text-gray-800 dark:text-white mt-2">{{ \App\Models\User::count() }}</h3>
                        </div>
                        <div class="text-5xl drop-shadow-sm opacity-90">👥</div>
                    </div>
                    <div class="mt-5 border-t border-gray-100 dark:border-gray-700 pt-4">
                        <a href="{{ route('admin.users.index') ?? '#' }}" class="text-sm font-bold text-blue-600 dark:text-blue-400 hover:text-blue-800 flex items-center transition-colors">
                            Quản lý tài khoản 
                ...
                <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700 p-6 transition-all duration-300 hover:shadow-lg hover:-translate-y-1">
                    <div class="flex items-center justify-between">
                        <div>
                            <p class="text-sm font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wider">Nhóm Hàng / Sản Phẩm</p>
                            <h3 class="text-3xl font-black text-gray-800 dark:text-white mt-2">{{ \App\Models\Category::count() }} <span class="text-xl text-gray-400 font-medium">/ {{ \App\Models\Product::count() }}</span></h3>
                        </div>
                        <div class="text-5xl drop-shadow-sm opacity-90">📦</div>
                    </div>
                    <div class="mt-5 border-t border-gray-100 dark:border-gray-700 pt-4">
                        <a href="{{ route('categories.index') ?? '#' }}" class="text-sm font-bold text-purple-600 dark:text-purple-400 hover:text-purple-800 flex items-center transition-colors">
                            Thiết lập kho bãi
                            <svg class="w-4 h-4 ml-1" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"></path></svg>
                        </a>
                    </div>
                ...
                <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700 p-6 transition-all duration-300 hover:shadow-lg hover:-translate-y-1">
                    <div class="flex items-center justify-between">
                        <div>
                            <p class="text-sm font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wider">Phiếu Nhập / Xuất</p>
                            <h3 class="text-3xl font-black text-gray-800 dark:text-white mt-2">{{ \App\Models\Receipt::count() }} <span class="text-xl text-gray-400 font-medium">/ {{ \App\Models\Issue::count() }}</span></h3>
                        </div>
                        <div class="text-5xl drop-shadow-sm opacity-90">🔄</div>
                    </div>
                    <div class="mt-5 border-t border-gray-100 dark:border-gray-700 pt-4">
                        <a href="{{ route('receipts.index') ?? '#' }}" class="text-sm font-bold text-green-600 dark:text-green-400 hover:text-green-800 flex items-center transition-colors">
                            Kiểm soát lịch sử
                            <svg class="w-4 h-4 ml-1" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"></path></svg>
                        </a>
                    </div>
                ...
                <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700 p-6 transition-all duration-300 hover:shadow-lg hover:-translate-y-1">
                    <div class="flex items-center justify-between">
                        <div>
                            <p class="text-sm font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wider">Giá Trị Tồn Kho</p>
                            <h3 class="text-2xl font-black text-gray-800 dark:text-white mt-2 truncate">
                                {{ number_format(\App\Models\Product::get()->sum(function($p) { return $p->quantity * $p->price; }), 0, ',', '.') }} <span class="text-lg text-gray-400">đ</span>
                            </h3>
                        </div>
                        <div class="text-5xl drop-shadow-sm opacity-90">💰</div>
                    </div>
                    <div class="mt-5 border-t border-gray-100 dark:border-gray-700 pt-4">
                        <a href="{{ route('admin.reports.index') ?? '#' }}" class="text-sm font-bold text-orange-600 dark:text-orange-400 hover:text-orange-800 flex items-center transition-colors">
                            Phân tích dữ liệu
                            <svg class="w-4 h-4 ml-1" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"></path></svg>
                        </a>
                    </div>
            ...
            @endif
            {{-- ========================================================================= --}}
            {{-- 2. GIAO DIỆN KHO & BIỂU ĐỒ (DÀNH CHO ADMIN VÀ THỦ KHO)                      --}}
            {{-- ========================================================================= --}}
                        ...
                        <div class="overflow-x-auto border border-gray-200 dark:border-gray-700 rounded-xl">
                            <table class="w-full table-fixed divide-y divide-gray-200 dark:divide-gray-700">
                                ...
                                <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                                    ...
                                    @empty
                                        <tr><td colspan="4" class="px-6 py-8 text-center text-gray-400">Tuyệt vời! Không còn đơn hàng nào tồn đọng.</td></tr>
                                    @endforelse
                                </tbody>
                            </table>
                        </div>
